...
public class Anvil116ColorMapping implements ColorMapping {
	...
	private static final Logger LOGGER = LogManager.getLogger(Anvil116ColorMapping.class);
	...
	public Anvil116ColorMapping() {
		...
		try (BufferedReader bis = new BufferedReader(
				new InputStreamReader(Objects.requireNonNull(Anvil116ColorMapping.class.getClassLoader().getResourceAsStream("mapping/116/colors.txt"))))) {
			String line;
		...
		try (BufferedReader bis = new BufferedReader(
			new InputStreamReader(Objects.requireNonNull(ColorMapping.class.getClassLoader().getResourceAsStream("mapping/116/biome_colors.txt"))))) {
			...
			throw new RuntimeException("failed to read mapping/116/biome_colors.txt");
		}
	}
	...
	public int getRGB(Object o, String biome) {
		throw new UnsupportedOperationException("color mapping for 1.16 does not support biome names");
	}
